...
(function($){
	...
	bValidator = function(mainElement, overrideOptions, instanceName){
		...
		var options = {
			...
			errorMessages: {
				en: {
					...
					'user_regex': 'Please enter alphanumeric characters and underscores only.',
					'user_taken': 'This username is already taken.',
					'email_taken':'This email has already been used.',
					'no_spaces':  'Spaces are not allowed in passwords.',
		...
		validator = {
			...
			user_regex: function(v){
				if(!v.value || !$.trim(v.value)){
					return true;
				}
				return this.regex(v, /^[A-Za-z0-9_]+$/i);
			},

			user_taken: function(v){
				ccn = $.cookie('csrf_cookie_name'); 
				username = v;
    			$.ajax({
    			type: "POST",
    			async: false,
    			url: 'http://localhost/yourslab_ci/register/username_check',
    			data: {
				'username' : username, 'csrf_token_name' : ccn 
				},
			...
			email_taken: function(v){
				ccn = $.cookie('csrf_cookie_name'); 
				email = v;
    			$.ajax({
    			type: "POST",
    			async: false,
    			url: 'http://localhost/yourslab_ci/register/email_check',
    			data: {
				'email' : email, 'csrf_token_name' : ccn 
				},
				success: function(data){
				if (data == 1) {
            	ret = false;
			...
			no_spaces: function(v){
				if(!v.value || !$.trim(v.value)){
				return true;
				}
				return this.regex(v, /^\S+$/i);
			},
